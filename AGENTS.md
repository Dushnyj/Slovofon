...
### 2.7 GitHub Actions — только релизы

По решению владельца GitHub Actions используется только для релизов.

- Единственный workflow — `.github/workflows/release.yml`: согласованный ручной релиз или push релизного тега `v*`.
- Не создавать и не запускать отдельные CI, Debug-сборки, проверки push/PR, scheduled jobs или другие нерелизные workflows.
- Обычные проверки, тесты и Debug-сборки выполнять локально до push.
- Внутри Release сохранять проверки качества, нативные тесты и проверки установщиков до публикации.
- Не восстанавливать удалённый CI без новой прямой команды владельца.
- Очистка истории Actions не означает разрешения удалять релизные запуски, Git tags, GitHub Releases или их файлы.
