...
## Дополнение: нативное закрытие Windows — 2026-09-08

На сборке `0.0.8+8` воспроизведён сбой после закрытия загруженного плеера:
`Windows.Media.dll / 0xc0000005`, затем `ucrtbase.dll / 0xc0000409`.
Скрытое parentless-окно `MediaPlayer SMTC window` считалось вторым окном
процесса. Поэтому стандартная проверка Flutter `IsLastWindowOfProcess`
пропускала запрос к `AppLifecycleListener`, а native players уничтожались
только при выгрузке DLL после завершения COM.

Главный runner теперь передаёт `WM_CLOSE` по отдельному lifecycle-каналу и
ждёт общего `PlaybackController.shutdown`. Подтверждение обрабатывается
отдельным posted message; повторные запросы объединяются, ошибки допускают
повтор, ответы после уничтожения окна игнорируются. SDK и зависимости не
подменялись. Стандартный Flutter exit API сохранён.

После пересборки проверено синтетическим Win32-вводом в **настоящую Windows
Debug-сборку**, не только widget-тестами:

- закрытие через `Alt+F4`: код завершения процесса **0**;
- повторный запуск: «Белые ночи», глава 3, пауза **05:32 / 11:02**, **26%**
  восстановлены;
- закрытие через крестик: код завершения процесса **0**.

Финальный локальный прогон после исправления: **1 997 passed, 20 skipped**;
`flutter analyze` без замечаний, `dart format` — 303 файла без изменений.

Отдельные Dart-тесты проверяют оба lifecycle-канала, ожидание native disposal,
ошибку/повтор и отсутствие такого teardown на Android/iOS. Native C++-тест
проверяет single-flight, устаревшие tokens, teardown и отказ `PostMessage`;
он включён в существующий Release workflow. Эта проверка не распространяется
на принудительное завершение процесса, выход из Windows или все сценарии
долговременного воспроизведения.
