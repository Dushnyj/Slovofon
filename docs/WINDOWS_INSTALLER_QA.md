...
4. В последующей доработке updater добавлен выбор MSI для текущей MSI-установки
   и запуск интерактивного `msiexec`; Setup больше не выбирается вместо MSI.
   Это подтверждается отдельными кодовыми/fixture-проверками, а не реальным
   MSI upgrade в этом отчёте. Подробности: [UPDATE_QA.md](UPDATE_QA.md).
