...
- Release notes — недоверенный Markdown, не HTML-страница. Нативный renderer
  не выполняет скрипты, не загружает изображения и не читает локальные файлы.
  Внешние ссылки требуют действия пользователя; допускаются только абсолютные
  `http`/`https` URL без credentials, пробелов/control characters и backslash.
  `javascript:`, `data:`, `file:`, `intent:` и другие схемы неактивны. Ссылка
  из текста никогда не становится источником установочного пакета или metadata.
  Отсутствие браузера/ошибка запуска не ломают текущий диалог. Длина отображаемого
  описания ограничена 64 000 символами; глубина обхода элементов — 24.
