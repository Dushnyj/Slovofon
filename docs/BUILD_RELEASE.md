...
#### Описание выпуска в приложении

`RELEASE_NOTES.md` остаётся единым Markdown-описанием для GitHub Release и окна
обновления Windows/Android/Android TV. В приложении оно отображается нативным
`ReleaseNotes`, а не строкой с символами разметки. Поддерживаются заголовки,
абзацы, маркированные и нумерованные списки (включая вложенные и task lists),
жирный/курсив/зачёркивание, inline/fenced code, цитаты, таблицы, ссылки и блоки
`[!NOTE]`, `[!TIP]`, `[!IMPORTANT]`, `[!WARNING]`, `[!CAUTION]`.

Правила подготовки описания:

- Краткий заголовок `# Slovofon vX.Y.Z`, затем короткое вступление и разделы `##`.
- Между абзацами, списками и alert-блоками — пустая строка. Перенос строки внутри
  одного абзаца не создаёт отдельную строку в интерфейсе.
- Для ссылок использовать `[понятное название](https://...)` с абсолютным URL.
  Якоря, относительные пути и ссылки на приложения/локальные файлы не открываются.
- Alert-маркер писать отдельной первой строкой цитаты: `> [!TIP]`, далее `> текст`.
  Внутри блока работают те же списки, ссылки и выделения.
- Предпочитать короткие списки и узкие таблицы. Код переносится по ширине окна,
  чтобы не требовать горизонтальной прокрутки на телефоне или пультом ТВ.
- Не рассчитывать на HTML/CSS, iframe, скрипты и изображения: HTML остаётся
  буквальным текстом, изображения заменяются alt-текстом без загрузки файлов.

Ссылки открываются только по действию пользователя во внешнем браузере. На ТВ
`↑/↓` на заголовке «Что нового» прокручивают текст, `→` переводит фокус к ссылкам,
`Select` открывает выбранную ссылку. Ошибка браузера не закрывает окно обновления.
Тема и масштаб текста общие с приложением; действия обновления остаются отдельно
от прокручиваемого описания. Отображаемое описание ограничено 64 000 символами.
...
3. Выбрать совместимый artifact именно этого release по имени, версии, платформе,
   ...
   при отсутствии подходящего artifact нельзя. Android использует только
   `Slovofon-v<version>-android-universal-release.apk`. Windows x64 сохраняет формат
   текущей запущенной копии: Setup — `Slovofon-v<version>-windows-x64-setup.exe`,
   MSI — `Slovofon-v<version>-windows-x64-msi.msi`, portable —
   `Slovofon-v<version>-windows-x64-portable.zip`. ZIP только скачивается и проверяется,
   без автоматической распаковки. ABI APK, AAB и MSIX не поддерживаются этим updater.
4. Для каждого распознанного artifact обязателен SHA256: использовать валидный `digest` API
   вида `sha256:<64 hex>`. Если digest не предоставлен, получить `SHA256SUMS.txt`
   **из того же release** и найти точную запись имени artifact. Проверка fail-closed:
   отсутствие суммы даже у другого поддержанного формата/платформы делает release
   ошибочным, а не разрешает частично доверенную установку. Нельзя
...
6. До открытия APK/установщика или папки с ZIP проверить точный размер и SHA256
   фактически скачанного файла. Несовпадение блокирует передачу файла дальше.
...
#### Lifecycle, выбор Windows-пакета и подтверждение пользователя

`UpdateStartupGate` проверяет release после первого кадра, при возвращении приложения
в foreground и каждые четыре часа непрерывной работы в foreground. Автопроверки имеют
30-минутный cooldown в текущем процессе; после закрытия приложения фонового сервиса
или системной задачи нет. Ручная кнопка **«Проверить новую версию»** в настройках
обходит cooldown и сохранённый пропуск версии. Одновременные запросы объединяются,
а диалоги принадлежат root Navigator; startup не использует контекст выше Navigator.
Ошибки автоматической проверки не открывают диалог; ручная проверка показывает ошибку
и действие **«Повторить»**, а не сообщает «Обновлений нет».

**«Пропустить версию»** сохраняет ключ версии в собственном `update-preferences.json`
в application-support каталоге. **«Позже»** только закрывает текущий диалог.
Пропуск переживает перезапуск, но не скрывает другую версию и не влияет на ручную
проверку. Этот файл отделён от БД, библиотеки и загрузок; чтение отсутствующего или
повреждённого файла не сбрасывает пользовательские данные. Ошибка сохранения видна
в диалоге и не изображается успешным пропуском.

На Windows native channel `com.slovofon.app/windows_update` / `getInstallationInfo`
проверяет регистрацию именно каталога запущенного `Slovofon.exe`, а не наличие любой
установленной копии. Используются read-only данные Setup/MSI и каталог текущего EXE;
неоднозначность, недоступность или timeout дают `unknown`. Контекст повторно читается
непосредственно перед запуском обработчика пакета.

| Контекст текущей копии | Действие в диалоге | Передача управления |
| --- | --- | --- |
| Setup | **«Обновить»** | Проверенный EXE, интерактивно: `/CURRENTUSER` либо `/ALLUSERS` и `/DIR=<текущий каталог>`. |
| MSI | **«Обновить»** | Системный `msiexec.exe /i <проверенный MSI> /norestart`, с интерфейсом установщика. |
| Portable | **«Скачать ZIP»** | Проверить ZIP и открыть его папку в Explorer. Пользователь закрывает приложение и распаковывает всю сборку в новую папку. |
| Unknown | **«Открыть страницу релизов»** | Открыть только фиксированную страницу GitHub Releases проекта; не скачивать и не запускать выбранный автоматически пакет. |

Если пакет известного формата отсутствует, другой формат не подставляется. До запуска
Windows Setup/MSI вызывается `PlaybackController.flushPlayback(requireSuccess: true)`
для сохранения последней позиции. Ошибка записи блокирует запуск и позволяет повтор.
Updater не использует silent install, не завершает приложение сам,
не заменяет работающий portable bundle и не удаляет настройки, прогресс или книги.
Сообщение об открытии установщика подтверждает только запуск процесса, не завершение
обновления; возможные elevation, закрытие занятых файлов и дальнейшие шаги принадлежат
интерактивному установщику и пользователю.

Это описание реализованного кода, а не подтверждение реального upgrade: установка
поверх предыдущего выпуска, UAC/отмена/rollback и перенос рабочей portable-копии
в этой итерации не выполнялись. Нужен отдельный Windows VM upgrade QA; новый релиз
или опубликованные артефакты самим изменением updater не создаются.
...
Встроенный updater выбирает **Setup EXE для текущей Setup-копии**, **MSI для MSI**
и **ZIP для portable**. Формат определяется по native регистрации каталога запущенного
EXE и повторно проверяется перед запуском; неизвестный контекст даёт только кнопку
перехода на страницу релизов. Setup сохраняет текущие scope и каталог; MSI запускается
интерактивно через системный `msiexec`. Для ZIP открывается папка проверенного архива,
а закрытие приложения и распаковка в новую папку остаются ручными действиями.
Полный контракт updater и граница непроведённого real-upgrade QA — в разделе 5.3.
