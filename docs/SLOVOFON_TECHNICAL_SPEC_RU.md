...
- Android: audio_service + just_audio либо другой совместимый стек
- Windows: использовать тот же engine, если он стабилен; если нет, реализовать WindowsAudioEngine через альтернативный пакет или platform channel
...
- service обращается к SourceConnector, Database, DownloadManager, AudioService.
...
UI, Android notification, lock screen, headset buttons, Windows mini-player и hotkeys должны обращаться к одному и тому же AudioService.
...
Media keys клавиатуры должны управлять AudioService.
