...
launcher и background media проверяется отдельно от наличия кода/widget-тестов.
См. [ANDROID_TV_TESTING.md](ANDROID_TV_TESTING.md), а визуальный контракт —
[THEMING.md, раздел 11](THEMING.md#11-android-tv-presentation-реализованный-контракт).

### 3.3 Windows

Основное окно имеет минимальную рабочую область 900×600 логических пикселей
(client DIPs, без системной рамки); стартовый normal размер — 1280×720 client.
Native runner учитывает DPI и рамку, не позволяет обойти минимум обычным resize
или восстановлением окна. Если доступная рабочая область монитора меньше,
размер вписывается в неё; ПК-оболочка сохраняется, контент прокручивается.
Это ограничение основного окна, а не будущего отдельного mini-player.
...
Статус на 2026-05-26: реализован Stage 7 connector в `lib/sources/izib/`. Поддержаны GraphQL search/details, SIGN generation на runtime, files -> chapters/audio tracks, resolveMedia/download через media allowlist и health check. Поиск Izib не содержит `files`, поэтому статус доступа в выдаче считается `unknown` до запроса details.
...
Реализовывать вторым реальным источником, после Izib.

Статус на 2026-05-27: реализован Stage 8 connector в `lib/sources/akniga/`. Поддержаны HTML search/details, `book id`/`bid`, `ajax/bid` track resolver, извлечение `LIVESTREET_SECURITY_KEY`, CryptoJS/OpenSSL-compatible AES-CBC hash, session cookies между page GET и ajax POST, Referer/media headers, media allowlist и `resolveMedia`/download path через существующий `SourceCatalogService`/`DownloadManager`.
...
**Статус реализации:** требования раздела остаются действующими. Сейчас используется
прямой `AudioMediaSource` с headers или локальный файл; `MediaProxyService`, его
интерфейс и localhost endpoint ещё не реализованы. Проверка разрешённых media hosts
и обновление ссылок в source layer не означают наличие локального proxy.
...
**Статус реализации:** пользовательские proxy profiles, `ProxyManager` и применение
выбранного proxy к metadata/media/downloads пока не реализованы. Описанный ниже
функционал остаётся требованием, а не заявлением о доступной настройке.
...
- адаптивный список начатых книг; в Windows-компоновке от 2026-09-05 — широкие строки с отдельными зонами метаданных, прогресса и действий;
- отдельная колонка/панель текущего плеера;
- быстрый доступ к загрузкам.

Рабочие экраны Windows используют всю доступную ширину после навигации, а не
узкий мобильный столбец. На главной рядом с крупной книгой расположены реальные
главы; в поиске рядом с основной областью — история и включённые источники.
Библиотека/загрузки используют широкие рабочие строки, настройки — inline-редактор
оформления и реальные параметры в адаптивных секциях,
подробности книги — summary слева и ленивый список содержимого справа.
При нехватке ширины и увеличенном тексте колонки последовательно переносятся;
никаких выдуманных рекомендаций/счётчиков для заполнения пространства.
Конкретные adaptive contracts и проверочные сценарии описаны в `THEMING.md`.
...
Текущая реализация `FileDownloadStorage` вычисляет полный SHA-256 от UTF-8
`jsonEncode([sourceId, bookVersionId])`. Читаемый безопасный префикс ограничен
24 символами и не участвует в определении уникальности пары. Это идентификатор
каталога, не проверка checksum аудио. Реальный файл обложки storage — `cover.img`;
расширения глав сохраняются согласно формату, временная глава имеет `.part`.

Ранее созданные каталоги `<safe(sourceId_bookVersionId)>` поддерживаются на месте
при точном совпадении `sourceId` и `bookVersionId` в metadata. Автоматического
перемещения, переименования или удаления старых пользовательских файлов нет.
Разрушительная операция не угадывает владельца legacy-каталога без проверяемой
metadata: такой случай требует восстановления. Схема локальной БД этим изменением
не меняется.
...
Акцентный цвет
Размер текста: 75–200%, шаг 5%, 100% — системный размер; предпросмотр при изменении
Компактные карточки
...
Статус реализации на 2026-05-26: этап 6 реализован как базовый framework без реальных сетевых источников.

```text
- SourceConnector interface добавлен в lib/sources/source_connector.dart.
- SourceRegistry добавлен в lib/sources/source_registry.dart.
- SourceCapabilities, SourceHealth, SourceException, SourceSearchResponse, SearchRequest, BookSearchResult и ResolvedMedia добавлены в lib/sources/source_models.dart.
- Shared parser helpers добавлены в lib/sources/source_parser_helpers.dart.
- Media validators добавлены в lib/sources/source_media_validator.dart.
- MockSourceConnector.yakniga() подключает существующие mock data для search/details/chapters/audio tracks/resolveMedia без сети.
- Покрытие тестами: test/sources/source_registry_test.dart, source_media_validator_test.dart, source_parser_helpers_test.dart, mock_source_connector_test.dart.
```

### Этап 7. Первый источник Izib
...
```text
- IzibSourceConnector добавлен в lib/sources/izib/ и экспортирован через lib/sources/sources.dart.
- IzibGraphQlClient генерирует SIGN на каждый request body и не отдаёт SIGN наружу.
- Mapper нормализует search/details/files в BookSearchResult, BookVersionDetails, Chapter и AudioTrack.
- resolveMedia валидирует URL через SourceMediaPolicy; произвольные media URL не принимаются.
- SourceCatalogService подключает Izib к app UI path: поиск, source details, chapters, playback book и download actions.
- SearchScreen использует реальный Izib-поиск; SourceBookDetailsScreen открывает source-книгу, показывает metadata/главы и запускает книгу или главу через PlaybackController.
- SearchScreen не запускает поиск во время набора текста, сохраняет историю запросов и фильтрует результаты по выбранному полю так, чтобы все слова запроса совпали как префиксы слов результата независимо от порядка (`Дыхание зоны`, `зоны дыхание`).
- Карточка результата Izib поддерживает icon-only действия: избранное, загрузка книги, play и открытие details; play/download показывают локальный circular loading, пока загружаются details/chapters/media.
- Избранное результата Izib сохраняется через LibraryStore/Drift, активное сердце отображается красным, а книга появляется в библиотеке без отдельного mock/fallback списка.
- После постановки книги в загрузки карточка показывает общий круговой прогресс с отменой всей книги; отмена удаляет queued/running tasks и уже скачанные части этой книги.
- SourceBookDetailsScreen для длинных книг показывает первые 5 глав по умолчанию и раскрывает остальные по явному действию пользователя.
- DownloadsScreen группирует задачи по книгам; главы показываются внутри раскрытой карточки книги, а не плоским списком из десятков глав. Карточка книги содержит общие pause/resume/retry/delete действия и показывает `скачано / полный размер`, только когда полный размер всех глав известен.
- HomeScreen не показывает поисковую плашку Izib, если уже есть активная книга для продолжения прослушивания.
- Source details открывается через stack navigation; системный Back и правый свайп возвращают пользователя на экран поиска.
- Source details учитывает нижний safe area Android, чтобы нижние информационные блоки и действия не перекрывались системной навигацией.
- Mini/full player и Downloads получают source metadata без fallback на unrelated mock book data.
- Unit-тесты Stage 7 покрывают GraphQL client/signing, mapper, connector, health и API errors.
- Widget/service тесты Stage 7 покрывают путь search -> details -> playback/downloads для Izib.
```

### Этап 8. Akniga

```text
- HTML search — реализовано
- book id/bid contract — реализовано
- ajax/bid resolver — реализовано
- security key extraction — реализовано
- track resolver — реализовано
- Referer/headers — реализовано
- media allowlist — реализовано
- search — реализовано
- details — реализовано
- chapters — реализовано
- resolveMedia — реализовано
- download — реализовано через общий DownloadManager path
- tests — реализованы fixture tests + optional live smoke
```
...
10. Есть хотя бы один реальный источник, лучше Izib.
