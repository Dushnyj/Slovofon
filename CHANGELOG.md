...
### Fixed
- Устранены случайные переносы отдельных букв в нижней навигации, включая «Библиотек / а». Подписи используют обычный межбуквенный интервал и измеряемую ширину пунктов; перенос разрешён только в предусмотренных локализацией местах. Добавлены типографические регрессии с настоящими шрифтами Roboto для пяти языков, узких экранов и увеличенного текста.

## [0.1.1] - 2026-09-08

### Fixed
- Нижняя мобильная навигация сохраняет все пять подписей под соответствующими иконками, включая длинные украинские названия. Перенос до двух строк заменяет общий режим с единственной подписью по центру; пользовательский масштаб текста, полные доступные названия и размеры целей нажатия сохранены.

## [0.1.0] - 2026-09-08

### Changed
- Каталоги главной, поиска, библиотеки и загрузок строятся лениво; скрытые экраны не перестраиваются на каждом тике плеера. Компоновка, масштаб текста и управление пультом сохранены.
- Поиск показывает готовые результаты отдельных источников, отменяет устаревшие сетевые запросы и выносит тяжёлый HTML-разбор из UI-потока. Регулировка громкости и прогресс загрузок больше не вызывают лишние записи базы и обновления интерфейса.
- Главная загружает metadata только книг из истории; загрузка одной книги и избранного не выполняет повторные запросы по всему каталогу.
- Полные каталоги интерфейса на казахском, белорусском и украинском языках вместо английского fallback. Локализованы счётные формы, длительность, размеры файлов и системные команды Android. В выбор языка добавлены локальные SVG-флаги единого формата 4:3; для языка системы используется нейтральная иконка.
- Единая политика анимаций: полные — короткие переходы и обратная связь; сниженные — минимальные изменения без пространственных перемещений; выключенные — конечное состояние сразу. Системное ограничение движения учитывается отдельно.
- Акцент согласован между кнопками, выбранными пунктами, вкладками, прогрессом, переключателями, ссылками и фокусом на всех платформах. Цвет текста рассчитывается относительно фактического фона; статусы ошибок и цвета источников сохраняют свой смысл.
- Android TV использует весь экран без внешней 4-процентной рамки; безопасные визуальные отступы находятся внутри содержимого. Внешний вид на TV разделён на компактные секции с заголовком и доступной кнопкой завершения.

### Fixed
- Исправлены некорректные обновления Windows-дерева доступности при появлении подсказок, смене экранов и открытии полного плеера; доступные названия и действия сохранены. При обычном выходе приложение дожидается сохранения данных, освобождения плеера и закрытия фонового подключения базы до завершения Dart VM.
- Эмблема Android TV banner предварительно уменьшается из оригинала в пять размеров плотности с качественным сглаживанием, вместо прямого уменьшения большого телефонного растра лаунчером. Компоновка и локализованные надписи сохранены.
- Повторный запуск Windows активирует уже открытое приложение, восстанавливает свёрнутое/скрытое окно и передаёт ссылки без создания второго плеера и подключения к базе.
- Фокус TV-транспорта сохраняется после перемотки и во время буферизации: повторные нажатия ±15 секунд не требуют возвращаться с кнопки паузы.
- Мобильные окна настроек открываются над мини-плеером и навигацией, учитывают доступную высоту и клавиатуру; действия остаются доступными при увеличенном тексте.
- Скруглённая заливка выбранных TV-пунктов и рамка используют один контур; прокрутка при фокусе учитывает верхнюю и нижнюю границы элемента.
- Холодный запуск Android TV дольше ожидает нативный профиль устройства, вместо преждевременного перехода к телефонной компоновке.

## [0.0.9] - 2026-09-08

### Fixed
- Окно обновления отображает Markdown из GitHub Releases с заголовками, списками, выделениями, кликабельными веб-ссылками и блоками Note/Tip/Important/Warning/Caution вместо исходной разметки. Оформление использует тему приложения на Windows, Android и Android TV; действия на телефоне скомпонованы компактнее. Поддержаны клавиатура и пульт, ссылки открываются во внешнем браузере. Новое оформление описаний доступно после установки 0.0.9.
- Перерисованы общие SVG-иконки перемотки на 15 секунд: центрированные векторные цифры, отчётливые треугольные наконечники и размер 28 логических пикселей без увеличения кнопок плеера. Единый вариант используется на Windows, Android и Android TV.

## [0.0.8] - 2026-09-08

### Changed
- Android TV получил компактную боковую навигацию и горизонтальные аудиокарточки с небольшой обложкой, автором/чтецом/циклом, доступными длительностью/годом/рейтингом/главами и цветным источником. Полные метаданные переносятся при крупном тексте; Select открывает подробности. Главы и информация разделены вкладками, полные действия книги, включая «Позже», остаются в подробностях. Настройки и загрузки уплотнены, мини-плеер показывает текущую главу, время и живой прогресс. Компоновка учитывает доступную логическую ширину и реальный масштаб текста, без подмены системной плотности Full HD/4K.
- TV-диалоги ограничены доступной высотой; выбор цвета доступен с пульта через пресеты и регуляторы. Карточки книги и поиск по автору/чтецу сохраняют TV-навигацию вместо телефонной нижней панели.
- README сокращён до страницы продукта: единая шапка с крупными значками релиза и платформ, возможности, галерея, скачивание и помощь. Подробные инструкции по установке и обновлению, включая ручной переход с v0.0.6 и выбор Android APK, собраны в руководстве пользователя; технические сведения и команды сборки оставлены в профильной документации.
...
### Fixed
- Исправлено закрытие Windows через крестик и Alt+F4: главное окно напрямую запрашивает сохранение позиции и освобождение нативного плеера, в том числе при наличии скрытого системного окна MediaPlayer. Повторные запросы объединяются; ошибка сохранения отменяет закрытие и позволяет повторить.
- Нумерация глав в карточке книги, плеере и загрузках начинается с 1 независимо от нулевых или непоследовательных индексов источника; сохранённые идентификаторы, индексы и позиция воспроизведения не меняются.
- На Android-планшетах крупный текст уменьшает число колонок вместо сжатия карточек; название приложения в боковой навигации остаётся целым при системном и пользовательском масштабе, включая RTL.
- Исправлены контраст выбранных пунктов сенсорной навигации и состояния ссылок автора/чтеца в мобильном и портретном плеере: текст и значки учитывают собственный фон, фокус и нажатие без ухудшающего контраст overlay.
- На TV исправлены переходы D-pad между содержимым, боковой навигацией и транспортом, выход из полей ввода и управление слайдерами без захвата вертикальных стрелок. Устранены нечитаемые состояния фокуса у источника, вкладок и громкости; карточки имеют деликатное отделение от фона и видимый фокус без изменения размеров.
- Android launcher использует адаптивную иконку с утверждённой эмблемой вместо белой оболочки legacy-значка. TV banner использует ту же эмблему, локализованное название и корректные пропорции; Android 7 сохраняет совместимые ресурсы.
- Иконки перемотки ±15 секунд унифицированы во всех плеерах: направление стрелки и читаемые векторные цифры больше не зависят от платформенного шрифта.
- Поиск по автору/чтецу из карточки корректно принимает результат на маршруте `/scoped-search`, вместо сохранения состояния загрузки после завершённого запроса.
- Контраст текста на пользовательском акценте рассчитывается по фактическому соотношению яркостей, включая оранжевые, золотые и средние серые оттенки.
- Добавлены проверки равной логической геометрии Windows при DPI 100/125/150/200%, Full HD и 4K; выравнивание логотипа учитывает RTL.
