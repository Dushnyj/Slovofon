...
### Changed
- Android TV получил более компактные меню, карточки и транспорт, адаптивные полки в несколько колонок, отдельный полный плеер с обложкой и двухколоночные настройки. Размеры зависят от логической области и пользовательского текста, а не физического разрешения Full HD/4K; системная плотность не подменяется.
...
- На TV исправлены переходы D-pad между содержимым, верхним меню и транспортом, выход из полей ввода и управление слайдерами без захвата вертикальных стрелок. Устранены нечитаемые состояния фокуса у источника, вкладок и громкости.
