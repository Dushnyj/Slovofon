...
Список отражает подключённые зависимости и ассеты; его нужно обновлять при добавлении стороннего компонента.

Лицензия самого проекта Slovofon указана в `LICENSE`. Attribution notice проекта указан в `NOTICE`.

Изображения для оформления репозитория в `docs/assets/readme/` используют
существующую иконку приложения и локальные Flutter-rendered fixtures.
Новые сетевые обложки, сторонние иллюстрации или файлы шрифтов не добавлялись.
Происхождение и параметры снимков описаны в [галерее README](docs/assets/readme/README.md).
